...
class MotorController:
    def __init__(self):
        ...
        if not versionFB[0] or not versionLR[0]:
            if not versionFB[0] and versionLR[0]:
                rospy.logfatal("GETVERSION Failed, Only Roboclaw FB")
            elif versionFB[0] and not versionLR[0]:
                rospy.logfatal("GETVERSION Failed, Only Roboclaw LR")
            else:
                rospy.logfatal("GETVERSION Failed, Both Roboclaws after trying switch")
            exit(1)
        else:
            print("RoboFB: " + repr(versionFB[1]) + ", RoboLR: " + repr(versionLR[1]))
            print("Car main battery voltage at start of script: ", self.rc.ReadMainBatteryVoltage(self.addressFB))
    ...
    def handle_speed(self, msg):
        """
        Receives the latest motor speed message and sends it to the roboclaws
        ...
        print "Forward: " + str(forward_left_motor)
        print "Right: " + str(right_turning_motor)
        print "Left: " + str(left_turning_motor)

        if forward_left_motor > 0:
            self.rc.ForwardM1(self.addressFB, self.constrain(forward_left_motor))
        else:
            self.rc.BackwardM1(self.addressFB, abs(self.constrain(forward_left_motor)))

        if forward_right_motor > 0:
            self.rc.ForwardM2(self.addressFB, self.constrain(forward_right_motor))
        else:
            self.rc.BackwardM2(self.addressFB, abs(self.constrain(forward_right_motor)))

        if right_turning_motor > 0:
            self.rc2.ForwardM1(self.addressLR, self.constrain(right_turning_motor))
        else:
            self.rc2.BackwardM1(self.addressLR, abs(self.constrain(right_turning_motor)))

        if left_turning_motor > 0:
            self.rc2.ForwardM2(self.addressLR, self.constrain(left_turning_motor))
        else:
            self.rc2.BackwardM2(self.addressLR, abs(self.constrain(left_turning_motor)))
